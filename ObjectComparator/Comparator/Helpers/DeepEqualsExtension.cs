...
public static class DeepEqualsExtension
{
    ...
    {
        var ignoreStrategy =
            ComparisonHelper.CreateIgnoreStrategy(Array.Empty<string>(), typeof(T).ToFriendlyTypeName());
        var customStrategies = strategies(new Strategies<T>());
        return DeeplyEquals(expected, actual, customStrategies.ToDictionary(x => x.Key, x => x.Value),
            ignoreStrategy);
    }
    ...
    /// <summary>
    ///     Performs a deep equality comparison between two collections with custom comparison strategies and properties to
    ///     ignore.
    /// </summary>
    /// <typeparam name="T">The element type of the collections.</typeparam>
    /// <param name="expected">The expected collection.</param>
    ...
    {
        return DeeplyEquals(expected, actual, new Dictionary<string, ICustomCompareValues>(), ignoreStrategy);
    }
    ...
    {
        return DeeplyEquals(expected, actual, new Dictionary<string, ICustomCompareValues>(), ignoreStrategy, options);
    }
